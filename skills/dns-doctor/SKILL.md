...
description: >-
  ...
  rejection, DKIM failures, a DNS change that may not have propagated yet, or a
  domain that might be blacklisted. Scans, fixes and verifies a domain's DNS —
  email authentication (SPF, DMARC, DKIM), multi-region propagation, SPF include
  supply-chain audits, MX, DNS health and domain/SSL expiry — against the DNS
  Doctor MCP server, and returns copy-paste fix records generated by a
  validating engine — never a guessed record.
license: Apache-2.0
...
7. **Confirm it went global.** `check_record` and your own resolver are one
   ...
   reads the same name from six vantage points on separate continents and
