---
name: dns-doctor
description: Use when a domain's email is landing in spam, or you see SPF PermError / "too many DNS lookups", DMARC stuck at p=none, a "550 5.7.515" or "550 5.7.1" rejection, DKIM failures, a DNS change that may not have propagated yet, or a domain that might be blacklisted. Scans, fixes and verifies a domain's DNS — email authentication (SPF, DMARC, DKIM), multi-region propagation, SPF include supply-chain audits, MX, DNS health and domain/SSL expiry — against the DNS Doctor MCP server, and returns copy-paste fix records generated by a validating engine — never a guessed record.
license: Apache-2.0
